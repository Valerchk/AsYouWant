...
/* ==========================================================================
   ...
   --------------------------------------------------------------------------
   One field, and under it a chip for each thing a block has: how long, when,
   and what it is for. The chips are not a second way of doing what the syntax
   does — they write the same string the parser reads, so "45m" typed and 45m
   tapped are the same edit and cannot contradict each other.

   The goal chip is the point of the whole thing. A goal used to have to exist
   before a block could belong to one, which put a tab of setup between a
   person and their first plan. Here a goal is born inside the composer, named,
   coloured and given its icon without leaving the day.
   ========================================================================== */
...
type Panel = "duration" | "time" | "goal";

export interface ComposerHandle {
  /** Drop text into the field and put the cursor after it. */
  prefill: (text: string) => void;
  /** Open the goal panel ready to name a new one. The day's goal strip uses
      this rather than carrying a second copy of the create flow. */
  newGoal: () => void;
}
...
export const Composer = forwardRef<ComposerHandle, Props>(function Composer(
  ...
  // Naming a new goal lives up here so the day's goal strip can start it.
  ...
  return (
    <form onSubmit={submit} className="w-full">
            ...
            <div className="max-h-[42dvh] overflow-y-auto pb-3">
              ...
              {panel === "goal" && (
                <GoalPanel
                  threads={threads}
                  selected={thread}
                  naming={naming || threads.length === 0}
                  onNaming={setNaming}
                  onPick={pickThread}
                  onCreate={(name) =>
                    onCreateThread(name)
                      ...
                      // The failure is already on screen: useDay puts the
                      // store's error where the day was.
                      .catch(() => {})
                  }
      ...
      {/* What the block will be, as three things you can change by tapping
          rather than by knowing the syntax. */}
      <div className="mt-2 flex flex-wrap items-center gap-1.5">
        ...
        >
          <Icon
            ...
            size={13}
          />
          {parsed.startMin === null ? "Flows" : formatClock(parsed.startMin)}
        </ChipToggle>
...
function GoalPanel({
  ...
  return (
    <div>
      <div className="flex flex-wrap gap-1.5">
        <Chip active={selected === null} onClick={() => onPick(null)}>
          None
        </Chip>
        {threads.map((t) => (
          <Chip
        ...
        {!naming && (
          <Chip active={false} onClick={() => onNaming(true)}>
      ...
      {/* Colour and icon, on the goal you just picked or just made — this is
          the whole reason a goal no longer needs a screen of its own. */}
      {selected && (
